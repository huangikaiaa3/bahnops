...
TABLE_CONFIG = {
    ...
    "poll_run": {
        "fields": ("poll_target_id", "started_at", "finished_at", "status", "services_seen", "error_message"),
        "on_conflict": {
            "use": False
        }
    },
    ...
    "service_state_current": {
        "fields": ("service_id", "latest_poll_run_id", "last_seen_at", "current_arrival", "current_departure", "current_arrival_platform", "current_departure_platform", "is_cancelled"),
        "on_conflict": {
            "use": True,
            "target": ("service_id",),
            "action": "DO UPDATE",
            "set": ("latest_poll_run_id", "last_seen_at", "current_arrival", "current_departure", "current_arrival_platform", "current_departure_platform", "is_cancelled"),
        },
    },
    "service_state_event": {
        "fields": ("service_id", "poll_run_id", "event_time", "event_type", "old_value", "new_value"),
        "on_conflict": {
            "use": False
        }
    }
...
def read_snapshot(path_to_snapshot: Path) -> dict:
    with path_to_snapshot.open("r", encoding="utf-8") as file:
...
def get_poll_target_data(station_id: int):
    return {
...
def get_service_data(snapshot: dict) -> dict:
    service_data = {}
    observed_at = snapshot["captured_at"]

    for this_service in snapshot["services"]:
        observation_uid = this_service["service_id"]
        service_run_uid, stop_sequence = observation_uid.rsplit("-", 1)

        service_data[observation_uid] = {
            "service_run_uid": service_run_uid,
            "category": this_service["category"],
            "number": this_service["number"],
            "line": this_service["line"],
            "display_name": this_service["display_name"],
            "observation_uid": observation_uid,
            "stop_sequence": int(stop_sequence),
            "observed_at": observed_at,
            "planned_arrival": this_service["planned_arrival"],
            "current_arrival": this_service["current_arrival"],
            "planned_departure": this_service["planned_departure"],
            "current_departure": this_service["current_departure"],
            "planned_arrival_platform": this_service["planned_arrival_platform"],
            "planned_departure_platform": this_service["planned_departure_platform"],
            "current_arrival_platform": this_service["current_arrival_platform"],
            "current_departure_platform": this_service["current_departure_platform"],
            "planned_arrival_path": json.dumps(this_service["planned_arrival_path"]),
            "planned_departure_path": json.dumps(this_service["planned_departure_path"]),
            "current_arrival_path": json.dumps(this_service["current_arrival_path"]),
            "current_departure_path": json.dumps(this_service["current_departure_path"]),
            "raw_snapshot": json.dumps(this_service),
        }

    return service_data

def _update_service_data(poll_run_id: int, service_data: dict, connection: psycopg.Connection) -> dict:
    for observation_uid, observation_data in service_data.items():
        _service_run_uid = observation_data["service_run_uid"]
        service_id = get_id_by_unique_field("service", "service_run_uid", _service_run_uid, connection)
        service_data[observation_uid]["poll_run_id"] = poll_run_id
        service_data[observation_uid]["service_id"] = service_id
        
    return service_data

def get_service_state_current_data(service_observation_data: dict) -> dict:
...
def _get_existing_service_state_current(service_id: int, connection: psycopg.Connection):
    query = """
...
def get_service_state_event_data(service_state_current_data: dict, connection: psycopg.Connection) -> list[dict]:
    ...
    for _, current_state_data in service_state_current_data.items():
        existing_state = _get_existing_service_state_current(current_state_data["service_id"], connection)
...
def write_snapshot_to_db(database_url, snapshot):
    ...
    with psycopg.connect(database_url) as connection:
        ...
        _station_id = get_id_by_unique_field("station", "eva_number", station_data["eva_number"], connection)
        poll_target_data = get_poll_target_data(_station_id)
...
def main() -> None:
    ...
    write_snapshot_to_db(database_url, snapshot)
